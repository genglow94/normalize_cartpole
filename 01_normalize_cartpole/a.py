...
for name, NetClass in modes.items():
    print(f"\n========== {name} モデル開始 ==========")
    final_avg_rewards = []
    converge_episodes = []
    avg100_list_all = []
    ...
    for seed in range(num_seeds):
        ...
        q_net = general_02(obs_dim, n_actions)
        target_net = general_02(obs_dim, n_actions)
        ...
        for episode in range(num_episodes):
            ...
            if episode % 100 == 0 and len(buffer) >= batch_size:
                ...
                g_func_fc = lambda x: q_net.fc(g_func_max1(x))
                g_func_min2 = lambda x: q_net.min2(g_func_fc(x))
                g_func_max2 = lambda x: q_net.max2(g_func_min2(x))

                # モードに応じて正規化
                if name in ["min_only", "both"]:
                    q_net.min1.restricted_normalize(D, f_func_fc_in, g_func_min1)
                    q_net.min2.restricted_normalize(D, g_func_fc, g_func_min2)

                if name in ["max_only", "both"]:
