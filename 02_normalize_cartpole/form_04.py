...
for mode in modes:
    ...
    for seed in range(num_seeds):
        ...
        # ======== ネットワーク ========
        q_net = form_04(obs_dim, n_actions).to(device)
        target_net = form_04(obs_dim, n_actions).to(device)
        ...
        for episode in range(num_episodes):
            ...
            if episode % 100 == 0 and len(buffer) >= batch_size:
             ...
             # ====== 関数構築（form_04 に対応） ======
               ...
               g_func_fc    = lambda x: q_net.fc(g_func_min1(x))
               g_func_max2  = lambda x: q_net.max2(g_func_fc(x))
               g_func_min2  = lambda x: q_net.min2(g_func_max2(x))

             # ====== モードに応じて正規化 ======
               if mode in ["min_only", "both"]:
                 q_net.min1.restricted_normalize(D, g_func_max1, g_func_min1)
                 q_net.min2.restricted_normalize(D, g_func_max2, g_func_min2)

               if mode in ["max_only", "both"]:
                 q_net.max1.restricted_normalize(D, f_func_fc_in, g_func_max1)
                 q_net.max2.restricted_normalize(D, g_func_fc, g_func_max2)
...
plt.title("Restricted Normalization form_04 DQN")
plt.xlabel("Episode")
